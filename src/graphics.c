...
extern Mesh graphicsMeshCreateFromObj(const s8* objPath)
{
	...
	num_indices *= 3;	// Each face has 3 indices

	Vertex* vertices = calloc(1, num_vertices * sizeof(Vertex));
	u32* indices = malloc(num_indices * sizeof(u32));
	s32 vertex_position_pos = 0, vertex_normal_pos = 0, index_pos = 0;
	u8 buffer[64];
	...
	while (index < objLen)
	{
		if (obj[index] == 'v')
		{
			...
			if (index < objLen && obj[index] == ' ')
			{
				...
				{
					assert(index < objLen);
					while (obj[index] == ' ' || obj[index] == '\t') ++index;

					s32 bufferPosition = 0;

					assert(index < objLen);
					while (isR32SuitableCharacter(obj[index]))
					{
						buffer[bufferPosition++] = obj[index];
						++index;
					}

					buffer[bufferPosition] = '\0';
					*((r32*)&(vertices[vertex_position_pos].position) + i) = (r32)atof(buffer);
				}

				vertices[vertex_position_pos].position.w = 1.0f;
				++vertex_position_pos;
			}
			else if (index < objLen && obj[index] == 'n')
			{
				++index;
				...
				if (index < objLen && obj[index] == ' ')
				{
					...
					for (s32 i = 0; i < 3; ++i)
					{
						...
						buffer[bufferPosition] = '\0';
						*((r32*)&(vertices[vertex_normal_pos].normal) + i) = (r32)atof(buffer);
					}

					vertices[vertex_normal_pos].normal.w = 0.0f;
					++vertex_normal_pos;
				}
			}
		}
		else if (obj[index] == 'f')
		{
			...
			if (index < objLen && obj[index] == ' ')
			{
				...
				for (s32 i = 0; i < 3; ++i)
				{
					...
					assert(index < objLen);
					while (isU32SuitableCharacter(obj[index]))
					{
						buffer[bufferPosition++] = obj[index];
						++index;
					}
					...
					buffer[bufferPosition] = '\0';
					indices[index_pos++] = (u32)atoi(buffer) - 1;

					while (obj[index] != ' ' && obj[index] != '\t' && obj[index] != '\n') ++index;
				}
			}
		}
		else
	...
	Vec4 color = (Vec4) { 1.0f, 0.0f, 0.0f, 1.0f }; // @temporary: temp color
...
{
	Vec4 color = entity->mesh.diffuseInfo.diffuseColor;

	glUseProgram(shader);
	GLint modelMatrixLocation = glGetUniformLocation(shader, "modelMatrix");
	GLint projectionMatrixLocation = glGetUniformLocation(shader, "projectionMatrix");
	GLint useTextureLocation = glGetUniformLocation(shader, "useTexture");
	GLint colorLocation = glGetUniformLocation(shader, "color");
	GLint texLocation = glGetUniformLocation(shader, "tex");
	glUniformMatrix4fv(modelMatrixLocation, 1, GL_TRUE, (GLfloat*)entity->modelMatrix.data);

	r32 bottom = 0;
	r32 left = 0;
	r32 right = (r32)orthoWidth;
	r32 top = (r32)orthoHeight;
	r32 far = 1;
	r32 near = -1;
	...
	glUniformMatrix4fv(projectionMatrixLocation, 1, GL_TRUE, (GLfloat*)orthoProjection.data);
	glUniform1i(useTextureLocation, entity->mesh.diffuseInfo.useDiffuseMap);

	if (entity->mesh.diffuseInfo.useDiffuseMap)
	{
		glUniform1i(useTextureLocation, true);
		glUniform1i(texLocation, 1);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, entity->mesh.diffuseInfo.diffuseMap);
	}
	else
	{
